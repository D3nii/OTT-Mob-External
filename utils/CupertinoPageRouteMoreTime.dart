...
mixin CupertinoRouteTransitionMixin<T> on PageRoute<T> {
  ...
  ValueNotifier<String> _previousTitle = ValueNotifier<String>('');
...
class CupertinoPageRouteMoreTime<T> extends PageRoute<T> with CupertinoRouteTransitionMixin<T> {
  /// Creates a page route for use in an iOS designed app.
  ///
  /// The [builder], [maintainState], and [fullscreenDialog] arguments must not
  /// be null.
  CupertinoPageRouteMoreTime({
...
class _CupertinoBackGestureDetectorState<T> extends State<_CupertinoBackGestureDetector<T>> {
  late _CupertinoBackGestureController<T> _backGestureController;
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    assert(mounted);
    _backGestureController.dragUpdate(_convertToLogical(details.primaryDelta! / (context.size?.width ?? 1)));
  }

  void _handleDragEnd(DragEndDetails details) {
    assert(mounted);
    _backGestureController.dragEnd(_convertToLogical(details.velocity.pixelsPerSecond.dx / (context.size?.width ?? 1)));
  }
  ...
  double _convertToLogical(double value) {
    ...
    return direction == TextDirection.rtl ? -value : value;
  }
...
class _CupertinoEdgeShadowDecoration extends Decoration {
  ...
  static const _CupertinoEdgeShadowDecoration none = _CupertinoEdgeShadowDecoration(edgeGradient: null);
  ...
  static _CupertinoEdgeShadowDecoration lerp(
    ...
    if (a == null && b == null) return const _CupertinoEdgeShadowDecoration(edgeGradient: null);
    return _CupertinoEdgeShadowDecoration(
      edgeGradient: a?.edgeGradient == null && b?.edgeGradient == null ? null : LinearGradient.lerp(a?.edgeGradient, b?.edgeGradient, t),
    );
  }
...
class _CupertinoEdgeShadowPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    if (gradient == null || configuration.size == null || configuration.textDirection == null) return;
